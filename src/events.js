...
const sayThankyou = ( event ) => {
  ...
  const thankyouMessages = [
    ...
    'No, gracias a ti',
    (
      '++ por tomarte el tiempo de decir gracias.\n...' +
      'no es cierto, es broma, no puedo darte `++`. Pero es la intención la que cuenta, ¿no?'
    )
  ];
...
const sendHelp = ( event ) => {
  ...
  const message = (
    ...
    '• `@usuario++`: Agrega puntos a un usuario o cosa\n' +
    '• `@usuario--`: Quita puntos a un usuario o cosa\n' +
    '• `@usuario++--`: ¿En realidad quieres intentar eso?, ¿acaso programas en HTML?\n' +
    '• `<@' + botUserID + '> leaderboard`: Muestra la tabla de líderes\n' +
    '• `<@' + botUserID + '> help`: Muestra este mensaje\n\n' +
    'Necesitas invitarme a un canal antes que pueda reconocer los ' +
    '`++` y `--`.\n\n' +
